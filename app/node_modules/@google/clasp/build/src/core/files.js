...
async function crawlLogicalSymlinks(base, current = base, recursive = true, visited = new Set()) {
    const results = [];
    try {
...
async function getLocalFiles(rootDir, ignorePatterns, recursive, allowSymlinks = false) {
...
export class Files {
    ...
    async WriteFiles(files, contentDir) {
        debug('Writing files');
        const absoluteContentDir = path.resolve(contentDir);
        const allowSymlinks = Boolean(this.options.files.allowSymlinks);
