...
export class ServerlessAuthorizationCodeFlow extends AuthorizationCodeFlow {
     ...
     */
    async promptAndReturnCode(authorizationUrl, expectedState) {
        ...
        if (!state || state !== expectedState) {
            const msg = intl.formatMessage({ id: "KVgUbE", defaultMessage: [{ type: 0, value: "Authorization rejected: state parameter mismatch. This may indicate a CSRF attack." }] });
            throw new Error(msg);
        }
        if (!code) {
            const msg = intl.formatMessage({ id: "vnSwYr", defaultMessage: [{ type: 0, value: "Missing code in response URL" }] });
            throw new Error(msg);
        }
        return code;
    }
