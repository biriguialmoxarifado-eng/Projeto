...
const STANDALONE_SCRIPT_TYPES = new Set(['standalone', 'webapp', 'api']);
export const command = new Command('create-script')
    ...
    if (!STANDALONE_SCRIPT_TYPES.has(type)) {
        ...
        if (!mimeType) {
            ...
            const msg = intl.formatMessage({ id: "U01p3W", defaultMessage: [{ type: 0, value: "Invalid script type \"" }, { type: 1, value: "type" }, { type: 0, value: "\". Valid types are: " }, { type: 1, value: "validTypes" }, { type: 0, value: "." }] }, { type, validTypes });
            this.error(msg);
        }
        ...
        if (!options.json) {
            ...
            if (type === 'webapp' || type === 'api') {
                ...
                const deploymentTip = intl.formatMessage({ id: "fmdW57", defaultMessage: [{ type: 0, value: "Tip: to deploy this script as a " }, { type: 1, value: "deploymentKind" }, { type: 0, value: ", configure \"" }, { type: 1, value: "manifestField" }, { type: 0, value: "\" in appsscript.json and run `clasp create-deployment`." }] }, { deploymentKind, manifestField });
